...
function [z0Mean, z0High, z0Low, z0Worst, tdecAll] = elimProbCtrlSing(k, Rmean, mu, rhoMean, ctrlMeth, saveFol, inp)
...
Rpdf = 0.0001:0.01:10; Rprob = zeros(lenk, length(Rpdf));
...
switch(ctrlMeth)
    ...
    case 2
        ...
        for i = 1:lenk
            ...
            %[trDist, trStat] = optGamTrunc(1, k(i), Rmean*rhoMean/k(i), rhoMean, Rmean);
    ...
    case 3
        ...
        for i = 1:lenk
            % Lower-truncated distribution and statistics
            [trDist, trStat] = optGamTruncBoth(2, k(i), Rmean*rhoMean/k(i), rhoMean, Rmean, 9);
...
if Rmean >= 1
    figStr = [num2str(round(Rmean)) '_' num2str(round(10*rhoMean)) '_' num2str(ctrlMeth)];
    figStr = join(['Rfixed' figStr]);
else
    figStr = [num2str(round(10*Rmean)) '_' num2str(round(10*rhoMean)) '_' num2str(ctrlMeth)];
