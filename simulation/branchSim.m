...
for ii = 1:lenk
    ...
    switch(ctrlMeth)
        ...
        case 3
            ...
            [trDist1, ~] = optGamTruncBoth(2, k(ii), Rs(1)/k(ii), 0.5, 2*Rs(1), 35);
    ...
    parfor j = 1:M
        ...
        Iwarn = 1;
        while Iwarn
            [Iday{j}, Iwarn] = branchSimDieCtrl(nday, ts, trDist1, trDist2);
        end
...
tdecmean = cellfun(@mean, tdecs); tdecvar = cellfun(@var, tdecs);
tdecmax = cellfun(@max, tdecs); 

% Check times of end
if all(tdecmean == t0mean) && all(tdecvar == t0var)
    disp('End times match');
end
...
legend(kstr, 'Location', 'best');
cd(saveFol);
saveas(gcf, ['varI_' namstr], 'fig');
cd(thisDir);

figure;
semilogx(k, t0mean, 'LineWidth', 2);
hold on;
semilogx(k, tdecmean, 'o', 'LineWidth', 2);
semilogx(k, t0var, 'LineWidth', 2);
semilogx(k, tdecvar, 'o', 'LineWidth', 2);
hold off; grid off; box off;
xlabel('$s$ (days)', 'FontSize', 18); 
ylabel('E[$t_0$], V[$t_0$]', 'FontSize', 18);
cd(saveFol);
saveas(gcf, ['t0_' namstr], 'fig');
cd(thisDir);
