...
function [z0Mean, z0High, z0Low, z0Worst, tdecAll, tdec0s] = empiricalProbCtrlFn(k, Rmean, mu, rhoMean, ctrlMeth, inp, idMeth)
...
nTraj = 2000; Rsamp = zeros(lenk, nTraj, zlen); 
% Pdf of R for plotting via cdf differences
Rpdf0 = 0:0.00001:100; lenp = length(Rpdf0) - 1;
% Using CDF differences to get probabilities
Rprob = zeros(lenk, lenp); Rpdf = Rpdf0(2:end);
...
switch(ctrlMeth)
    case 1
        ...
        for i = 1:lenk
            ...
            % Cumulative density function of adjusted R
            Rcdf = gamcdf(Rpdf0, k(i), Rmean*rhoMean/k(i));
    ...
    case 2
        ...
        for i = 1:lenk
            ...
            [trDist, trStat] = optGamTruncTest(1, k(i), Rmean*rhoMean/k(i), rhoMean, Rmean, 0.1);
            %[trDist, trStat] = optGamTruncIncomp(1, k(i), Rmean*rhoMean/k(i), rhoMean, Rmean);
    ...
    case 3
        ...
        for i = 1:lenk
            % Lower-truncated distribution and statistics
            [trDist, trStat] = optGamTruncTest(2, k(i), Rmean*rhoMean/k(i), rhoMean, Rmean, 30);
            %[trDist, trStat] = optGamTruncIncomp(2, k(i), Rmean*rhoMean/k(i), rhoMean, Rmean);
...
% Check normalisation
Rnorm = sum(Rprob, 2)'; disp(['Normalisation = ' num2str(Rnorm)]);
...
if saveFig
    cd(saveFol);
    clear Rsamp z0s Rpdf Rpdf0 Rprob
